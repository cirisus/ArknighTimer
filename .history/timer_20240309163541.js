...
function createTimer(language, targetTime, progressMessage, finishedMessage) {
  ...
  var updateInterval;
  switch (initialRemaining.highest) {
    case YEARS_ENUM:
      updateInterval = DAYS_MS; // update every day
      break;
    case MONTHS_ENUM:
    case DAYS_ENUM:
      updateInterval = HOURS_MS; // update every hour
      break;
    case HOURS_ENUM:
      updateInterval = MINUTES_MS; // update every minute
      break;
    case MINUTES_ENUM:
    case SECONDS_ENUM:
      updateInterval = SECONDS_MS; // update every second
      break;
  }
...
function setup() {
  ...
  if (!timestamp) {
    setError('未设置时间','对应的参数是"time"，可以使用UNIX时间戳或ISO时间字符串。');
    return;
  }
