...
function createTimer(language, startTime, targetTime, startMessage, progressMessage, finishedMessage) {
  startMessage = startMessage || getMessage(language, 'timer-unstarted');
...
function updateProgressBar(startTimestamp, targetTime) {
  ...
  function updateTimer() {
    ...
    if (now < startTime) {
      ...
      titleElement.innerText = startMessage;
...
function setup() {
  ...
  var startMessage = parameters.get('unstarted');
  ...
  function parseTimestamp(timestamp) {
    ...
    if (/^\d+(\.\d+)?$/.exec(timestamp)) {
      time = new Date(parseFloat(timestamp) * 1000);
    } else {
      time = new Date(timestamp);
      if (isNaN(time)) {
        setError('无效时间戳', timestamp);
        return null;
      }
    }
    return time;
  ...
  // Check parameters
  if (!language) {
    setError('未指定语言', '缺少参数"lang"，使用"cn"来指定中文计时器。');
    return;
  }
  if (!startTimestamp) {
    setError('未设置开始时间','缺少参数"start"，接收UNIX时间戳或ISO时间字符串。');
    return;
  }
  var startTime = parseTimestamp(startTimestamp);
  if (!startTime) return;

  if (!timestamp) {
    setError('未设置结束时间','缺少参数"time"，接收UNIX时间戳或ISO时间字符串。');
    return;
  }
  var targetTime = parseTimestamp(timestamp);
  ...
  // Initialize clock
  createTimer(language, startTime, targetTime, startMessage, progressMessage, finishedMessage);
}
